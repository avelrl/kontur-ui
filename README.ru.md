...
Живое демо: [avelrl.github.io/kontur-ui](https://avelrl.github.io/kontur-ui/)
...
- Это не полностью продуктовый пакет дизайн-системы со стабильным API компонентов.
...
- Это не публичный npm-пакет и он не обещает стабильность внешнего API за пределами экспортированного theme entry.
...
- Переиспользуйте семантические токены из [`src/styles/theme.css`](./src/styles/theme.css).
...
## Как Подключить Тему В Другом Проекте

После `npm run build` репозиторий собирает переиспользуемый theme-слой в `dist/theme/`.
Экспортируемый `theme.css` это Tailwind CSS v4 source layer для внутренних или workspace-потребителей, а не готовый standalone stylesheet.
...
- `theme.css` с семантическими токенами, обеими версиями темы и маппингом токенов для Tailwind v4
- `kontur-theme.js` с runtime-хелперами
- `kontur-theme.d.ts` с TypeScript-типами
- Продуктовые тексты и locale runtime остаются на стороне consuming application
...
Переиспользуемые токены темы лежат в [`src/styles/theme.css`](./src/styles/theme.css):
...
Демо-слой layout и chrome остаётся в [`src/styles/app.css`](./src/styles/app.css), который импортирует theme source layer.
...
- [`src/lib/locale.tsx`](./src/lib/locale.tsx) управляет текущим языком, его сохранением и `html[lang]` только внутри демо.
- [`src/data/locale.ts`](./src/data/locale.ts) хранит локализованные тексты интерфейса демо для `ru` и `en`.
...
1. Добавьте или скорректируйте семантические токены в [`src/styles/theme.css`](./src/styles/theme.css).
